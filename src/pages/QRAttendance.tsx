...
export default function QRAttendance() {
  ...
  useEffect(() => {
    if (!sessionId) {
      toast.error("No session ID provided. Redirecting to Create Session...");
  ...
  const attendanceRate = sessionData?.class_strength > 0 ? Math.round((scannedStudents.length / sessionData.class_strength) * 100) : 0;

  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', width: '100%' }}>
        <div style={{ width: '24px', height: '24px', border: '3px solid rgba(14,165,200,.3)', borderTopColor: '#0ea5c8', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
    ...
    <div style={{ width: "100%", minHeight: "100vh", fontFamily: "'Sora', 'Inter', sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
        ...
        <GlowCard style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "20px 22px" }} glowColor="rgba(59, 130, 246, 0.3)">
          <div style={{ fontSize: 32, fontWeight: 800, letterSpacing: "-1px", lineHeight: 1, color: "#fff" }}>{sessionData?.class_strength || 0}</div>
          ...
          <div style={{ padding: "22px 24px" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16, paddingTop: 10 }}>
              ...
              <div style={{ position: "relative", padding: 16, borderRadius: 16, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(14,165,200,0.4)", boxShadow: "0 0 50px rgba(14,165,200,0.32)", animation: "glow 2.5s infinite" }}>
                ...
                <QRCodeSVG value={`${window.location.origin}/attendance?session=${sessionId}&code=${currentCode}&v=${qrVersion}`} size={220} bgColor="#1a2436" fgColor="#0ea5c8" level="M" />
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, overflow: "hidden" }}>
            ...
            <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: 0 }}>
              {[
                { label: "Subject", value: sessionData?.subject || "—" },
                { label: "Room", value: sessionData?.room || "—" },
                { label: "Department", value: sessionData?.department || "—" },
                { label: "Started", value: sessionData?.start_time || "—" },
            ...
            <div style={{ maxHeight: 320, overflowY: "auto" }}>
              {scannedStudents.length === 0 ? (
                <div style={{ padding: "28px 18px", textAlign: "center", color: "rgba(255,255,255,0.4)", fontSize: 13, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                  ...
                  <span>Students will appear<br />here as they scan</span>
                </div>
              ) : (
